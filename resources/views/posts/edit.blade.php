...
<body class="antialiased">
    <div class="content p-4">
        <form class="flex flex-col w-2/12" action="{{ route('posts.update', $Post->id)}}" method="post" enctype="multipart/form-data">
            @csrf
            ...
            value="{{ $Post->title }}"
                ...
                <textarea class="textarea textarea-primary" name="body" placeholder="Bio">{{ $Post->body}}</textarea>
                @error('body')
                <div class="text-red-500 text-sm">{{ $message }}</div>
                @enderror
